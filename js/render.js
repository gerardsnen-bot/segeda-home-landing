...
(function () {
  ...
  function buildProductCard(product) {
    ...
    var badgesHtml = product.badges.map(function (b) {
      return '<span class="badge badge-' + escapeHtml(b.type) + '">' + b.label + '</span>';
    }).join('\n            ');

    var ctaHref = 'https://wa.me/51978642447?text=' + encodeURIComponent(product.ctaMessage);
    ...
    return '<article class="' + articleClass + '" id="' + escapeHtml(product.id) + '" aria-label="' + escapeHtml(product.name) + '">\n' +
      '        <div class="product-img" style="background: ' + product.imageBg + '; padding:0;">\n' +
      '          <img src="' + escapeHtml(product.image) + '" alt="' + escapeHtml(product.imageAlt) + '" loading="lazy" style="width:100%; height:100%; object-fit:cover; display:block;">\n' +
      '        </div>\n' +
